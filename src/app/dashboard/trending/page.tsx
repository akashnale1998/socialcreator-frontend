...
import { TrendingUp, ArrowUpRight, Globe, BarChart2, Lock, Sparkles, ArrowRight, Copy, CheckCircle2 } from 'lucide-react';
    ...
    return (
      <div className="max-w-4xl mx-auto py-20 text-center space-y-8">
        <UpgradeModal 
          ...
          featureName="Trending Topics"
          requiredPlan="Creator Plan"
        />
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      <UpgradeModal 
        ...
        featureName="Trending Topics"
        requiredPlan="Creator Plan"
        message="Trending Topics is only available for Creator and Pro users. Upgrade to see what's going viral right now."
      />
             ...
             <Card className="h-full group hover:border-primary/20">
                <div className="flex items-start justify-between mb-6">
                 ...
                 <div className="flex items-start justify-between gap-4 mb-4">
                    <h3 className="text-xl font-bold group-hover:text-primary transition-colors pr-8">{trend.topic}</h3>
                    <button 
                      onClick={() => handleCopy(trend.topic, trend.topic)}
                      className={`p-1.5 rounded-lg transition-all ${
                        copiedId === trend.topic 
                        ? 'bg-emerald-500/20 text-emerald-500' 
                        : 'text-white/20 hover:text-white hover:bg-white/5 opacity-0 group-hover:opacity-100'
                      }`}
                    >
                      {copiedId === trend.topic ? <CheckCircle2 size={16} /> : <Copy size={16} />}
                    </button>
                 </div>

...
import { generateHookThunk, analyzeHookThunk, generateHookSlice, analyzeHookSlice } from '@/store/apis';
...
export default function HookGenerator() {
  ...
  useEffect(() => {
    dispatch(generateHookSlice.actions.reset());
    dispatch(analyzeHookSlice.actions.reset());
  }, [dispatch]);

  const handleGenerate = async () => {
    if (!topic) return;
    trackEvent('generate_hook', 'Tool', 'Generated Hooks');
    dispatch(generateHookThunk({ topic, platform, audience, hookStyle, language }));
  };

  const handleAnalyze = async () => {
    if (!hookToAnalyze) return;
    trackEvent('analyze_video', 'Tool', 'Analyzed Hook for Viral Score');
                ...
                {results.length === 0 && !isProcessing ? (
                  ...
                  <>
                                ...
                                <div className="flex items-start justify-between gap-4">
                                  ...
                                  <div className="flex flex-col gap-2">
                                    <button
                                      ...
                                      title="Copy Hook"
                                    >
                                      {copiedId === `hook-${index}` ? <CheckCircle2 size={18} /> : <Copy size={18} />}
                                    </button>
                    ...
                    <div className="flex flex-col items-center justify-center py-8 gap-4 border-t border-white/5">
                      <p className="text-sm text-white/40">Need more hooks? Try generating another set.</p>
                      <Button 
                        onClick={handleGenerate} 
                        isLoading={isProcessing}
                        ...
                        variant="glass"
                      >
                        <Zap size={18} className="text-amber-400" /> Generate More (1 Credit)
                      </Button>
                    </div>
                  </>
                )}
                      ...
                      <div className="space-y-3">
                        {(analysis.improvedHooks || analysis.improved || []).map((hook: string, i: number) => (
                          <div key={i} className="p-4 rounded-xl bg-white/5 border border-white/5 group hover:border-primary/30 transition-all flex items-center justify-between gap-4">
                            ...
                            <button 
                              onClick={() => handleCopy(hook, `improved-${i}`)}
                            ...
                            >
                              {copiedId === `improved-${i}` ? <CheckCircle2 size={16} /> : <Copy size={16} />}
                            </button>
                          </div>
                        ))}
                      </div>
